    ...
    req.on("end", async () => {
        try {
            ...
            if (!passRegex.test(password)) {
                return json(res, 400, {
                    error: "Пароль должен содержать одну заглавную букву, одну строчную, одну цифру и один спецсимвол.",
                });
            }
            if (password) {
                return json(res, 400, {
            ...
            sendEmail(email, "Регистрация в Nova App", `Здравствуйте, ${name}!
        ...
        С уважением,  
        Команда Nova App`).catch(console.error); // не ломаем регистрацию, если письмо не ушло
